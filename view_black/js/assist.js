...
$(document).ready(function() {
	...
	currentWindow.setSkipTaskbar(false);
			...
			ipcRenderer.on('stopLive',function(event){
				Server.stopAVConf(classroomId,getUrlParam("token"),function(){
			...
			ipcRenderer.on('closeFayanrenVideo',function(event){
				sendToPeer("broadcast", JSON.stringify({"key":"closeFayanrenVideo"}));
			});
			/*//推题传白板图片
...
/*			$('div').resize(function(){	
		...
		function showWin(allVideos){
			...
			{	var is_have = false;
				...
				//if(snId[0] == 1)main_stream_index=snId[1];
				...
				if(!is_have)
				{
					...
					{
						$('#'+snId[0]+' div').html("<img src='../img/img02.jpg' />");
					}
					else
					{
						$('#'+snId[0]+' div').html("<img src='../img/img04.jpg' />");
					}
		...
		function appendLi12(classroomArr)
		{
				...
				str +='<li id="'+speaker_id+'" sn_id="'+zjr_snId+'" class="zhujiangren"><h4>'+zjr_title+'</h4><div class="main"><img src="../img/img02.jpg" /></div>';
		...
		function append_li(classroomArr,is12){
				...
				for(var j=0;j<classroomArr.length;j++)
				{
					...
					liObj.find("h4").html(classroomArr[j].client_name);
